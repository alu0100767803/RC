...
for punto in objetivos:
  while distancia(tray_ideal[-1],punto) > EPSILON and len(tray_ideal) <= 1000:
    ...
    #### Modificación ####
    medida = ideal.sense(objetivos)   #Distancias del robot ideal a los objetivos
    prob = real.measurement_prob(medida, objetivos)         #Calculamos la probabilidad del robot real respecto del ideal
    print(prob);
    if(prob < 0.01):  #Si la probabilidad es muy baja localizamos 
      localizacion(objetivos, real, ideal, [ideal.x, ideal.y], 3) #Localizamos el robot real 
    ###########
